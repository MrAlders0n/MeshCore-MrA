...
void MyMesh::logRxRaw(float snr, float rssi, const uint8_t raw[], int len) {
  ...
  CONSOLE_SERIAL.print(getLogDateTime());
  CONSOLE_SERIAL.print(" RAW: ");
  mesh::Utils::printHex(CONSOLE_SERIAL, raw, len);
...
void MyMesh::handleCommand(uint32_t sender_timestamp, char *command, char *reply) {
  ...
  if (memcmp(command, "setperm ", 8) == 0) {   // format:  setperm {pubkey-hex} {permissions-int8}
    ...
    CONSOLE_SERIAL.println("ACL:");
    for (int i = 0; i < acl.getNumClients(); i++) {
      auto c = acl.getClientByIdx(i);
      if (c->permissions == 0) continue;  // skip deleted (or guest) entries

      CONSOLE_SERIAL.printf("%02X ", c->permissions);
      mesh::Utils::printHex(CONSOLE_SERIAL, c->id.pub_key, PUB_KEY_SIZE);
